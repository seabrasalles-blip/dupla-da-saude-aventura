...
const STEPS = [
  ...
  { icon: "👣", title: "Arraste o personagem", desc: "Leve Nina ou Nino até a casa destacada." },
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 space-y-2 text-slate-700 text-base max-w-3xl">
        <p>Este jogo é para <b>2 jogadores</b>.</p>
        <p>
          Na sua vez, jogue o dado, avance pelo tabuleiro e resolva o desafio da casa. Cada resposta
          certa ajuda a dupla a ganhar <b>selos de cuidado</b>.
        </p>
        <p>
      ...
      <div className="mt-6 grid grid-cols-5 gap-3">
        {STEPS.map((s, i) => (
          <div
            key={i}
            className="rounded-2xl bg-white border-4 border-slate-200 p-4 text-center shadow-md"
          >
            <div className="text-5xl">{s.icon}</div>
            <div className="mt-2 font-bold text-slate-800">{i + 1}. {s.title}</div>
      ...
      <div className="mt-8 flex gap-3">
        ...
        <Link
          to="/"
          className="rounded-full bg-white border-2 border-slate-400 text-slate-700 font-bold px-6 py-3"
        >
          Voltar
        </Link>
      </div>
